...
The example data is fictional, and it was built with two genuine routes to a
high outcome: `infrastructure * knowledge`, and `finance * talent`. Because
either route is enough on its own, no single condition should come out as
necessary. Keep that in mind as a benchmark for what follows.

## The short version

If you only want the results, `run_pfsqca()` performs every step in this
vignette in a single call:
...
res                        # console summary
export_pfsqca(res)         # CSV tables
pfsqca_report(res)         # HTML report
```

The rest of this vignette walks through the same analysis step by step, which
is what you want when you need to justify each decision in a paper.
...
Only the variables listed in `vars` are transformed; `case_id` and `period` are
carried through untouched. Calibration uses the pooled panel, so a membership
score of 0.8 means the same thing in every period.
